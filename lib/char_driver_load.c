...
static void p_setup_cdev(struct p_driver *p_driver)
{
	...
	if(p_driver->p_dev)
	{
		...
		for(i=0; i<p_driver->limit; i++)
		{
			...
			printk("<1>++++driveropen device p_setup-cdev on %d %d. [ok]\n",
					dev, i);
			...
			if(err)
				printk
				(
					"<1> Error %d adding \"driver_open\" in p_setup_cdev %d\n",
					err, p_driver->limit
				);
		
			printk
			(
				"<1> success %d adding i=%d\"driver_open\" in p_setup_cdev %d [ok]\n",
				err, i, p_driver->limit
			);
